...
void MyDataStore::dump(std::ostream &ofile) {
    ...
    for(set<Review *>::iterator singleReview = reviewList_.begin(); singleReview != reviewList_.end(); singleReview++){
        ofile << (*singleReview)->prodName << '\n' << (*singleReview)->rating << (*singleReview)->username << (*singleReview)->date
...
std::vector<std::string> MyDataStore::userNames(){
    std::vector<std::string> names;
    for(set<User *>::iterator singleUser = userList_.begin(); 
        singleUser != userList_.end(); singleUser++){
        string tempName = (*singleUser)->getName();
...
void MyDataStore::setCurrentUser(string userNameInput){
    for(set<User *>::iterator user = userList_.begin(); user != userList_.end(); user++){
        if((*user)->getName() == userNameInput){
            currentUser_ = (*user);
        }
    }
}
