...
bool myfunction (std::pair<std::string, double> one, std::pair<std::string, double> two) {
...
std::vector<std::pair<std::string, double> > MyDataStore::makeSuggestion(std::string currentUser) {
    ...
    for (auto singleProd : notReviewed) {
        ...
        for (auto singleUser : userList_) {
            if(singleUser->getName() == currUser->getName()){
                continue;
            }
    ...
    sort (buyThese.begin(), buyThese.end(), myfunction);
    return buyThese;
}
